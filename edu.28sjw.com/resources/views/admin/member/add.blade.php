	...
	<form action="" method="post" class="form form-horizontal" id="form-member-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				...
				<div class="radio-box">
					<input type="radio" id="type-2" name="status" value="2"  checked>
...
$(function(){
	//在选择国家后列出省份数据，是change事件,通过属性选择器选中name为country_id的属性
	$('select[name=country_id]').change(function(){
		...
		$.get('/admin/member/getareabyid',{id: id},function(data){
			//jQuery循环操作
			var str = '';
			//each是jQuery的循环操作方法，有两个参数，data是循环的数据，和回调函数，index是当前循环到元素的索引，el当前循环到的元素
			$.each(data,function(index,el){
				str += "<option value='" + el.id + "'>" + el.area + "</option>";
			});
			//追加之前清除之前的二级之后的数据,gt对数据过滤大于0的
			...
			//将数据放到对应的option后，append方法是将str放到select名字为province_id的后面里面的位置
			$('select[name=province_id]').append(str);
		},'json')
	});

	//在选择省份后列出城市数据，是change事件,通过属性选择器选中name为country_id的属性
	...
	//在选择城市后列出县区数据，是change事件,通过属性选择器选中name为country_id的属性
